import { _decorator, Component, Node, Font, BitmapFont, Vec2, ImageAsset, Texture2D, Asset, RenderTexture, MeshRenderer, Material, Vec3, Size, utils, geometry, EventHandler, Label } from 'cc';
...
export class Label3D extends Component {
    ...
    onEnable() {
        this._assemblerData = Label._canvasPool.get();
        this.updateRenderData();
    }
    start() {
        this.updateRenderData();
    ...
    */
    ...
    onDisable() {
        if (this._assemblerData) {
            Label._canvasPool.put(this._assemblerData);
        }
