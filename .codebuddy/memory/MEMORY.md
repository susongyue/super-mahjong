...
## 版本管理约定
- 每次完成实质性改动后，将修改内容追加写入 `CHANGELOG.md`（按版本号分段），未推送时在当前版本号下追加
- **不要自行 commit 或 push**：仅在用户明确下发推送指令后，才执行 commit + push
- 版本推送 commit 格式：`vX.Y.Z - 总结内容：有哪些修改`（推送后递增版本号，默认 +0.0.1）
- 当前版本：**v1.1.0**
